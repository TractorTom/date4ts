                    ...
                    for (lenB in list_len[-1L]) {
                        ...
                        testthat::test_that(test_name, {
                            ...
                            if (lagB > 0L) {
                                ...
                                } else if (lagB + lenB < lenA) {
                                    # Cas 6
                                    ts_ResAB1 <- ts(
                                        c(
                                            A_content[1L:lagB],
                                            valueB,
                                            A_content[(lagB + lenB + 1):lenA]
                                        ),
                                        start = startA,
                                        frequency = frequenceA
                                    )
                                ...
                                if (lenB < lenA) {
                                    ts_ResAB1 <- ts(
                                        c(valueB, A_content[(lenB + 1):lenA]),
                                        start = startA,
                                        frequency = frequenceA
                                    )
                                    ts_ResAB2 <- combine2ts(ts_A, ts_B)
...
testthat::test_that("NA values generate warning", {
    for (typeA in list_type[-6L]) {
        ...
        v1 <- sample(
            c(
                create_random_type(typeA, len = 10L),
                create_NA_type(type = typeA, len = 5L)
            ),
            replace = TRUE
        )
        ...
        testthat::expect_warning(
            set_value_ts(series = ts_A, date_ts = 2010L, replacement = v1),
            regexp = "Contains missing values"
        )
        testthat::expect_warning(
            set_value_ts(series = ts_A, date_ts = 2010L, replacement = v2),
            regexp = "Contains missing values"
        )
    }
})
