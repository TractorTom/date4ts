...
for (typeA in list_type[-c(5L, 7L)]) {
...
testthat::test_that("date_from must be before date_to", {
    for (year in good_years) {
        for (len in list_len[-1L]) {
            ...
            date_1 <- year
            date_2 <- year + len
            testthat::expect_error(get_value_ts(
                ts_A,
                date_from = date_2,
                date_to = date_1
            ))
