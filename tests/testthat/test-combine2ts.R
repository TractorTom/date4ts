...
liste_len <- c(0L, 1L, 5L, 10000L)
liste_frequence <- c(1, 2, 4, 7, 12, 0.1, 1/3, 3.5, 365.25, pi)
liste_start <- c(-200, -2, 0, 2021, -0.2, 2018 + sqrt(3))
...
#                                            "type", typeA,
#                                            "frequency", round(frequenceA, 2L),
#                                            "start", round(startA, 6L),
#                                            "lenA", lenA,
#                                            "param1", param1,
#                                            "param2", param2)
...
#                                 ts_B2 <- ts(B2_content,  start = ts4conj::getTimeUnits(start(ts_A), frequency = frequenceA) - param1 / frequenceA, frequency = frequenceA)
#
...
#                                 ts_B3 <- ts(B3_content,  start = ts4conj::getTimeUnits(start(ts_A), frequency = frequenceA) - (param1 + param2) / frequenceA, frequency = frequenceA)
#
#                                 if (typeA == "raw"){
...
#
#                                 if (typeA == "raw"){
#                                     ts_ResAB4 <- ts(c(A_content, rep(as.raw(0L), param1), B4_content), start = start(ts_A), frequency = frequenceA)
#                                     ts_ResB4A <- ts(c(A_content, rep(as.raw(0L), param1), B4_content), start = start(ts_A), frequency = frequenceA)
#                                     if (param1 > 0L){
#                                         testthat::expect_warning({
...
#                                     }
#                                 } else {
#                                     ts_ResAB4 <- ts(c(A_content, rep(NA, param1), B4_content), start = start(ts_A), frequency = frequenceA)
#                                     ts_ResB4A <- ts(c(A_content, rep(NA, param1), B4_content), start = start(ts_A), frequency = frequenceA)
#                                     testthat::expect_warning({ resAB4 <- combine2ts(ts_A,ts_B4)},
#                                                              regexp = "extending time series when replacing values")
#                                     testthat::expect_warning({ resB4A <- combine2ts(ts_B4,ts_A)},
#                                                              regexp = "extending time series when replacing values")
...
#                             ts_B5 <- ts(B5_content,  start = ts4conj::getTimeUnits(start(ts_A), frequency = frequenceA) - param1 / frequenceA, frequency = frequenceA)
#                             ts_ResAB5 <- ts_B5
#
...
#                                 ts_B6 <- ts(B6_content,  start = ts4conj::getTimeUnits(start(ts_A), frequency = frequenceA) + param1 / frequenceA, frequency = frequenceA)
#                                 if (param1 == 0L){
#                                     ts_ResAB6 <- ts(c(B6_content, A_content[(param1 + param2 + 1L):lenA]), start = start(ts_A), frequency = frequenceA)
#                                 } else {
#                                     ts_ResAB6 <- ts(c(A_content[1L:param1], B6_content, A_content[(param1 + param2 + 1L):lenA]), start = start(ts_A), frequency = frequenceA)
#                                 }
#                                 ts_ResB6A <- ts_A
#
...
testthat::test_that("miscellaneous input are not allowed", {
    for (objA in object_bank_R){
        ...
        for (objB in object_bank_R){
            testthat::expect_error(combine2ts(objA, ts_B), regexp = "Les objets a et b doivent être des ts unidimensionels.")
        }
    }
})
...
testthat::test_that("arguments are temporally consistent", {
    ...
    ts_A <- create_random_type(type = "num", len = 20L) |> ts(start = 2015L, frequency = pi)
    ts_B <- create_random_type(type = "num", len = 20L) |> ts(start = 2016L, frequency = pi)
    testthat::expect_error(combine2ts(ts_A, ts_B), regexp = "Les objets a et b doivent être cohérents temporellement.")
    testthat::expect_error(combine2ts(ts_B, ts_A), regexp = "Les objets a et b doivent être cohérents temporellement.")
