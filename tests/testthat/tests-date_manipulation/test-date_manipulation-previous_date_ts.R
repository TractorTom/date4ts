    ...
    for (year in double_years) {
        for (lag in list_lag) {
            ...
            testthat::expect_warning(
                object = {res <- previous_date_ts(date_a, frequency_ts = 12L, lag = lag)},
...
testthat::test_that(desc = "Warning with double years and quaterly frequency", {
    ...
    for (year in double_years) {
        for (lag in list_lag) {
            ...
            testthat::expect_warning(
                object = {res <- previous_date_ts(date_a, frequency_ts = 4L, lag = lag)},
                regexp = message_double("date_ts")
            )
...
testthat::test_that(desc = "Warning with frequency and monthly frequency", {
    ...
    for (year in good_years) {
        for (lag in list_lag) {
            ...
            testthat::expect_warning(
                object = {res <- previous_date_ts(date_a, frequency_ts = 12, lag = lag)},
...
testthat::test_that(desc = "Warning with frequency and quaterly frequency", {
    ...
    for (year in good_years) {
        for (lag in list_lag) {
            ...
            testthat::expect_warning(
                object = {res <- previous_date_ts(date_a, frequency_ts = 4, lag = lag)},
                regexp = message_double("frequency_ts")
            )
