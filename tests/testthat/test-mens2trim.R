...
testthat::test_that("detection of wrong dates", {
    for (wrong_date in wrong_dates) testthat::expect_error(mens2trim(wrong_date))
})
...
testthat::test_that("warning for integer date", {
    for (good_year in good_years) {
        for (warning_month in warning_integer_months) {
            ...
            testthat::expect_warning({resQuarterly <- mens2trim(c(good_year, warning_month))}, regexp = "Assertion on 'period' failed: Element 1 is not <= 12.|Assertion on 'period' failed: Element 1 is not >= 1.")
...
testthat::test_that("warning for double date", {
    for (warning_year in double_years) {
        for (good_month in good_months) {
            ...
            testthat::expect_warning({resQuarterly <- mens2trim(c(warning_year, good_month))}, regexp = "Assertion on 'date_ts' failed: Must be of type 'integer', not 'double'.")
    ...
    for (good_year in good_years) {
        for (warning_month in double_months) {
            ...
            testthat::expect_warning({resQuarterly <- mens2trim(c(good_year, warning_month))}, regexp = "Assertion on 'date_ts' failed: Must be of type 'integer', not 'double'.")
