...
La fonction `combine2ts` combine (comme c()) 2 time series de même
...
celles de a sur la période concernée. Si il existe une période sur
laquelle ni `a` ni `b` ne prennent de valeur (mais qu'il existe des
valeurs à des dates ultérieures et antérieures) alors le ts en sortie
prendra NA sur cette période.
